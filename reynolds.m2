...
Eijm = (i0,j0,m) -> (matrix apply(
    m, i -> apply(
        m, j -> if (i==i0 and j==j0) then (
            1/1;
        ) else (
            0/1
        );
...
slnBasis = (n) -> (
    ...
    Xbasis := flatten apply(
        n, i -> delete(null,apply(n, j -> if i<j then Eijm(i,j,n)))
    );   
    Ybasis := flatten apply(
        n, i -> delete(null,apply(n, j -> if i>j then Eijm(i,j,n)))
    ); 
...
slnDualBasis = (n) -> (
    ...
    XDualbasis := flatten apply(
        n, i -> delete(null,apply(n, j -> if i<j then Eijm(j,i,n)))
    );   
    YDualbasis := flatten apply(
        n, i -> delete(null,apply(n, j -> if i>j then Eijm(j,i,n)))
    );
    L := flatten {HDualbasis, XDualbasis, YDualbasis};
    return apply(L, i -> (1/(2*n))*i);
)
