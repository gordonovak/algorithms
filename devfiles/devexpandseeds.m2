...
-- METHOD_NAME: degCompare
exponentCompare = method();
exponentCompare := (func, divisor) -> (
    expFunc = exponents(func);
    expDiv = exponents(divisor);

    for i to (#expFunc - 1) do (
        maxDiv = max apply(expDiv, l -> l_i);
        if (maxDiv == 0 and max apply(expFunc, l -> l_i) != 0) then (
            return false;
        );
    );
...
devexpandseeds = method();
devexpandseeds (List, List) := (L, ZList) ->(
    ...
    for i when (i < #M) do (
        ...
        candidate = M_i;
        for j when (j < #M) do (
            if (i != j and exponentCompare(candidate, M_j) and (candidate % M_j) == 0) then (
